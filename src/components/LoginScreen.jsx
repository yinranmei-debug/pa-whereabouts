  ...
  return (
    <div style={{minHeight:'100vh',width:'100%',display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center',padding:'24px',textAlign:'center',position:'relative',overflow:'hidden',background:'#f8fafc',fontFamily:"'Plus Jakarta Sans',sans-serif"}}>
      <style>{`
        ...
        .ls-orb{position:absolute;border-radius:50%;filter:blur(80px);z-index:0;animation:cloudFloat 12s ease-in-out infinite;opacity:0.35;}
        ...
        .ls-card{
          ...
          box-shadow:0 40px 100px -20px rgba(148,163,184,0.2),inset 0 0 0 1px rgba(255,255,255,0.5);
        }
      ...
      <div style={{position:'relative',zIndex:10,width:'100%',maxWidth:900,transition:'all 1s',opacity:show?1:0,transform:show?'translateY(0)':'translateY(40px)'}}>
        <div style={{marginBottom:48}}>
          ...
          <p className="ls-sub" style={{color:'#94a3b8',fontSize:'clamp(14px,2vw,18px)',fontWeight:500,margin:0}}>
        ...
        <div className="ls-card" style={{width:'100%',maxWidth:480,margin:'0 auto',borderRadius:52,padding:'48px 56px',display:'flex',flexDirection:'column',alignItems:'center'}}>
          <img src="https://i.ibb.co/YTQHg15F/Pattern-Logo.png" alt="Pattern" style={{height:44,objectFit:'contain',opacity:0.85,marginBottom:32}}/>
          <h2 style={{fontSize:20,fontWeight:700,color:'#1e293b',letterSpacing:'-0.02em',marginBottom:32}}>Welcome Back</h2>

          {isInitializing ? (
            <div style={{display:'flex',flexDirection:'column',alignItems:'center',gap:16,padding:'24px 0'}}>
              <div className="ls-spinner"/>
              <div style={{fontSize:13,fontWeight:600,color:'#94a3b8',letterSpacing:'0.04em'}}>Initializing session...</div>
            </div>
          ) : (
            <>
              <button onClick={onLogin} className="ls-btn" style={{width:'100%',height:64,background:'#0f172a',color:'#fff',borderRadius:16,fontSize:15,fontWeight:700,display:'flex',alignItems:'center',justifyContent:'center',gap:16,boxShadow:'0 8px 24px rgba(15,23,42,0.2)'}}>
              ...
              {error && (
                <div style={{marginTop:20,padding:'10px 16px',background:'#fef2f2',borderLeft:'4px solid #f87171',color:'#b91c1c',fontSize:13,borderRadius:'0 8px 8px 0',textAlign:'left',width:'100%'}}>
                  {error}
                </div>
              )}
            </>
          ...
          <div style={{width:'100%',paddingTop:28,marginTop:32,borderTop:'1px solid #f1f5f9',display:'flex',alignItems:'center',justifyContent:'center'}}>
            <div style={{display:'flex',alignItems:'center',gap:8}}>
              ...
              <span style={{fontSize:11,fontWeight:700,letterSpacing:'0.12em',color:'#94a3b8',textTransform:'uppercase'}}>Whereabouts</span>
            </div>
          </div>
        </div>
      </div>
