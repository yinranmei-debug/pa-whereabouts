...
const BreachCanvas = memo(({ phaseRef, progressRef, isChargingRef }) => {
  ...
  const animate = useCallback((now) => {
    ...
    if (isCharging && progress > PORTAL_APPEAR_AT && phase !== 'EXPLODING' && phase !== 'IMPLODING') {
      ...
      if (now - lastSpawnRef.current > spawnInterval && particles.current.length < MAX_PARTICLES) {
        ...
        particles.current.push({
          x: sx, y: sy,
          vx: perpX * tangentialSpeed - (dx / dist) * 0.5,
    ...
    if (phase !== 'EXPLODING') window.__breachBurstFired = false;
    ...
    for (let i = particles.current.length - 1; i >= 0; i--) {
      ...
      if (phase === 'EXPLODING' && p.kind === 'burst') {
        p.x += p.vx; p.y += p.vy;
        p.vy += 0.06; p.vx *= 0.992; p.vy *= 0.992;
        p.life -= 0.004;
        ...
        p.x += p.vx; p.y += p.vy;
        p.size *= 0.92; p.life -= 0.02;
        if (dist < 20) p.life = 0;
      } else {
        const dx = cx - p.x; const dy = cy - p.y;
        const dist = Math.sqrt(dx * dx + dy * dy) + 1;
        const holeRadius = Math.max(60, progressRef.current * 3);
        const shredZone = holeRadius * 1.2;
  ...
  return (
    <canvas ref={canvasRef} style={{ position:'fixed', inset:0, pointerEvents:'none', zIndex:11400 }} />
  );
});
...
export default function DimensionalBreachOverlay({ breach, chargingState }) {
  ...
  const quakeRef = useRef({ portal: { x:0, y:0 }, text: { x:0, y:0 } });
  ...
  useEffect(() => {
    ...
    if (breach?.phase === 'FLOATING') currentSizeRef.current = 0;
  }, [breach, isCharging]);
  ...
  useEffect(() => {
    const loop = () => {
      ...
      let target = 0;
      if (breach?.phase === 'EXPLODING') target = explodingMaxSize;
      ...
      el.style.filter = shouldFilter ? 'url(#breach-turbulence)' : 'none';
      let opacity = 0;
      if (breach?.phase === 'EXPLODING') opacity = 1;
      else if (breach?.phase === 'IMPLODING') opacity = newVal > 5 ? Math.min(1, newVal / 200) : 0;
      else if (isCharging && chargingProgress >= PORTAL_APPEAR_AT) opacity = Math.min(1, (chargingProgress - PORTAL_APPEAR_AT) / 4);
  ...
  useEffect(() => {
    const loop = () => {
      ...
      let portalX = 0, portalY = 0, textX = 0, textY = 0;
      if (ph === 'EXPLODING') {
        portalX = (Math.random() - 0.5) * 10; portalY = (Math.random() - 0.5) * 10;
        textX = (Math.random() - 0.5) * 18; textY = (Math.random() - 0.5) * 18;
      } else if (ph === 'IMPLODING') {
        portalX = (Math.random() - 0.5) * 4; portalY = (Math.random() - 0.5) * 4;
      ...
      if (portalShakeRef.current) portalShakeRef.current.style.transform = `translate3d(${q.portal.x}px,${q.portal.y}px,0)`;
      if (textShakeRef.current) textShakeRef.current.style.transform = `translate(-50%,-50%) translate3d(${q.text.x}px,${q.text.y}px,0)`;
  ...
  return (
    <>
      ...
      {showAnything && (
        <>
          <div style={{
            position:'fixed', inset:0, zIndex:11350, pointerEvents:'none',
            ...
            opacity: isExploding
              ...
              : isCharging ? Math.min(0.65, (chargingProgress/100)*0.75) : 0,
            transition:'opacity 400ms ease-out',
          }}/>

          {showPortal && (
            <div style={{ position:'fixed', inset:0, display:'flex', alignItems:'center', justifyContent:'center', pointerEvents:'none', zIndex:11600 }}>
              <div ref={portalShakeRef} style={{ willChange:'transform' }}>
                <div ref={portalRef} style={{
                  width:0, height:0, borderRadius:'50%', position:'relative',
                  background:`radial-gradient(circle at 50% 50%, #000 0%, #0a0420 25%, #1e1b4b 50%, #4c1d95 72%, #a855f7 88%, rgba(192,132,252,0.6) 96%, transparent 100%)`,
                  boxShadow:`0 0 60px rgba(168,85,247,0.6), 0 0 120px rgba(79,70,229,0.45), 0 0 200px rgba(99,102,241,0.3), inset 0 0 100px rgba(0,0,0,0.6)`,
                  willChange:'width, height, opacity',
                }}>
                  <div style={{ position:'absolute', top:'2%', left:'2%', right:'2%', bottom:'2%', borderRadius:'50%', border:'2px solid transparent', borderTopColor:'#a855f7', borderLeftColor:'#a855f7', animation:'breachSpin 1.2s linear infinite', opacity:0.9 }}/>
                  <div style={{ position:'absolute', top:'10%', left:'10%', right:'10%', bottom:'10%', borderRadius:'50%', border:'2px solid transparent', borderBottomColor:'#60a5fa', borderRightColor:'#60a5fa', animation:'breachSpin 1.8s linear infinite reverse', opacity:0.7 }}/>
                  <div style={{ position:'absolute', top:'20%', left:'20%', right:'20%', bottom:'20%', borderRadius:'50%', background:'radial-gradient(circle, rgba(255,255,255,0.3) 0%, transparent 70%)', animation:'breachPulse 2.2s ease-in-out infinite' }}/>
                </div>
              </div>
            </div>
          )}
          ...
          {isCharging && chargingProgress >= PORTAL_APPEAR_AT && (
            <div style={{
              ...
              zIndex:11650, pointerEvents:'none', textAlign:'center',
              animation:'chargingFadeIn 400ms ease-out',
            }}>
              <div style={{
                fontFamily:"'Plus Jakarta Sans',sans-serif",
                fontSize:11, fontWeight:800, letterSpacing:'0.5em', textTransform:'uppercase',
                color:'rgba(199,210,254,0.9)',
                textShadow:'0 0 20px rgba(129,140,248,0.6), 0 2px 10px rgba(0,0,0,0.8)',
                marginBottom:6,
              }}>
                {chargingProgress < 30 ? 'energy accumulating' : chargingProgress < 75 ? 'the rift is widening' : 'almost there'}
              </div>
              <div style={{
                fontFamily:"'Plus Jakarta Sans',sans-serif",
                ...
                {chargingProgress < 30 ? 'keep going...' : chargingProgress < 75 ? 'pressure building' : 'breakthrough imminent'}
              </div>
            </div>
          )}
          ...
          {isCharging && (
            <div style={{
              position:'fixed', bottom:0, left:0, right:0, zIndex:11680, pointerEvents:'none',
              padding:'20px 32px 24px 32px',
              background:'linear-gradient(to top, rgba(0,0,0,0.7) 0%, transparent 100%)',
              animation:'energyBarFadeIn 500ms ease-out',
            }}>
              <div style={{ display:'flex', justifyContent:'space-between', alignItems:'baseline', maxWidth:1600, margin:'0 auto 10px auto' }}>
                <div style={{ display:'flex', alignItems:'baseline', gap:8 }}>
                  <span style={{
                    fontFamily:"'Plus Jakarta Sans',sans-serif", fontWeight:900, fontStyle:'italic',
                    fontSize:'clamp(32px,4vw,54px)', lineHeight:1,
                    background: isNearFull ? 'linear-gradient(90deg,#fde047 0%,#c084fc 100%)' : 'linear-gradient(90deg,#a5b4fc 0%,#c084fc 100%)',
                    WebkitBackgroundClip:'text', WebkitTextFillColor:'transparent', backgroundClip:'text',
                    filter:'drop-shadow(0 0 20px rgba(168,85,247,0.6))', letterSpacing:'-0.03em',
                  }}>{Math.round(pr)}%</span>
                  <span style={{ fontFamily:"'Plus Jakarta Sans',sans-serif", color:'rgba(199,210,254,0.7)', fontSize:10, fontWeight:800, letterSpacing:'0.3em', textTransform:'uppercase' }}>Energy</span>
                </div>
                <div style={{ display:'flex', alignItems:'center', gap:8, fontFamily:"'Plus Jakarta Sans',sans-serif" }}>
                  <span style={{ width:8, height:8, borderRadius:'50%', background: chargingUserCount>=2 ? '#a855f7' : '#6366f1', boxShadow:'0 0 12px currentColor', display:'inline-block' }}/>
                  <span style={{ color:'rgba(255,255,255,0.9)', fontSize:12, fontWeight:800, letterSpacing:'0.25em', textTransform:'uppercase' }}>
                    {chargingUserCount >= 2 ? `${chargingUserCount} teammates` : 'solo'}
                  </span>
                </div>
              </div>
              <div style={{ maxWidth:1600, margin:'0 auto', height:10, background:'rgba(255,255,255,0.08)', borderRadius:999, overflow:'hidden', boxShadow:'inset 0 1px 2px rgba(0,0,0,0.6)' }}>
                <div style={{
                  width:`${pr}%`, height:'100%',
                  ...
                  borderRadius:999,
                  boxShadow: isNearFull ? '0 0 20px rgba(168,85,247,0.8)' : '0 0 12px rgba(99,102,241,0.6)',
          ...
          {(breach?.phase === 'EXPLODING' || breach?.phase === 'IMPLODING') && (
            <div style={{ position:'fixed', inset:0, display:'flex', alignItems:'center', justifyContent:'center', pointerEvents:'none', zIndex:11700, padding:'40px' }}>
              <div ref={textShakeRef} style={{
                position:'absolute', top:'50%', left:'50%',
                transform:'translate(-50%,-50%)',
                opacity: breach.phase==='IMPLODING' ? 0 : 1,
                transition:'opacity 500ms ease-out', willChange:'transform',
              }}>
                <div className="breach-pop" style={{ textAlign:'center', maxWidth:1200 }}>
                  <h2 style={{
                    ...
                    letterSpacing:'-0.04em', lineHeight:0.95, margin:'0 0 32px 0',
                  ...
                  <p style={{
                    fontFamily:"'Plus Jakarta Sans',sans-serif",
                    color:'rgba(255,255,255,0.85)', fontSize:'clamp(16px,2vw,28px)',
                    fontWeight:300, fontStyle:'italic', letterSpacing:'0.02em',
                    maxWidth:720, margin:'0 auto', textShadow:'0 2px 10px rgba(0,0,0,0.8)',
                  }}>
      ...
      <style>{`
        @keyframes breachSpin { from{transform:rotate(0deg)} to{transform:rotate(360deg)} }
        @keyframes breachPulse { 0%,100%{opacity:0.5;transform:scale(0.9)} 50%{opacity:1;transform:scale(1.1)} }
        @keyframes breachPop {
          0%{transform:scale(0.3);opacity:0} 15%{transform:scale(1.1);opacity:1}
          20%{transform:scale(1)} 85%{transform:scale(1);opacity:1} 100%{transform:scale(1.2);opacity:0}
        }
