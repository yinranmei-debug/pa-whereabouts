...
export default function DimensionalBreachOverlay({ breach, chargingState }) {
  ...
  // ============================================================
  // 🌀 SUCK ENGINE — hijacks existing DOM confetti
  // Scans for elements with class `breach-suckable` and applies
  // gravity toward the center when charging/imploding.
  // ============================================================
  useEffect(() => {
    // Per-element state stored on the DOM node itself (tiny closure)
    const suckState = new WeakMap();
    ...
    const loop = () => {
      ...
      // Only active when something is happening
      const active =
        ph === 'IMPLODING' ||
        (charging && pr > 15); // start sucking after ~15% progress

      if (active) {
        const cx = window.innerWidth / 2;
        const cy = window.innerHeight / 2;

        // Gravity strength scales with progress/phase
        let gravityStrength;
        if (ph === 'IMPLODING') {
          gravityStrength = 2500; // very strong
        } else {
          // Charging: 0 at progress=15, ramps up to ~600 at progress=100
          gravityStrength = Math.pow((pr - 15) / 85, 1.4) * 800;
        }

        document.querySelectorAll('.breach-suckable').forEach((el) => {
          let state = suckState.get(el);
          if (!state) {
            ...
            // Disable the original CSS transition so we take over
            el.style.transition = 'none';
            suckState.set(el, state);
          }
          ...
          state.vx = (state.vx + (dx / dist) * force * 0.05) * 0.92;
          state.vy = (state.vy + (dy / dist) * force * 0.05) * 0.92;

          // Falling gravity still applies a little bit during charging
          if (ph !== 'IMPLODING') {
            state.vy += 0.3;
          }
          ...
          // Apply position via translate (relative to element's current position)
          // We use left/top directly instead of transform to avoid conflicting
          // with the original animation's rotate
          el.style.left = `${state.x}px`;
          el.style.top = `${state.y}px`;
        });
      } else {
        // Not active: clear any state on elements that disappeared
        // (no action needed; confetti will just complete their normal animation)
      }
  ...
  useEffect(() => {
    const loop = () => {
      ...
      const targetDisp = isCharging ? 20 + chargingProgress * 0.6 : breach?.phase === 'EXPLODING' ? 80 : 20;
