...
// Vibe 飞行魔法：飞行图层组件
const EmojiFlyLayer = ({ flight, onComplete }) => {
  const [progress, setProgress] = useState(0);
  const [pathPoints, setPathPoints] = useState([]);

  useEffect(() => {
    if (!flight) {
      setProgress(0); setPathPoints([]); return;
    }
    const duration = 900;
    ...
    const animate = (now) => {
      ...
      if (p < 0.25) {
        ...
        const control = { x: start.x + (end.x - start.x) * 0.2, y: Math.max(start.y, end.y) + 180 };
        curX = Math.pow(1 - ease, 2) * start.x + 2 * (1 - ease) * ease * control.x + Math.pow(ease, 2) * end.x;
        curY = Math.pow(1 - ease, 2) * start.y + 2 * (1 - ease) * ease * control.y + Math.pow(ease, 2) * end.y;
        currentPoints.push({ x: curX, y: curY });
        setPathPoints([...currentPoints]);
      }
      if (p < 1) requestAnimationFrame(animate);
      else setTimeout(onComplete, 50);
    };
    requestAnimationFrame(animate);
  }, [flight, onComplete]);

  if (!flight) return null;
  const { start, progress: pLocal } = { start: flight.start, progress };
  let x, y, scale, rotate;
  if (pLocal < 0.25) {
    const t = pLocal / 0.25;
    const ease = 1 - Math.pow(1 - t, 3);
    x = start.x; y = start.y; scale = ease * 2; rotate = t * -20;
  } else {
    const t = (pLocal - 0.25) / 0.75;
    const ease = t < 0.5 ? 8 * t * t * t * t : 1 - Math.pow(-2 * t + 2, 4) / 2;
    const control = { x: start.x + (flight.end.x - start.x) * 0.2, y: Math.max(start.y, flight.end.y) + 180 };
    x = Math.pow(1 - ease, 2) * start.x + 2 * (1 - ease) * ease * control.x + Math.pow(ease, 2) * flight.end.x;
    y = Math.pow(1 - ease, 2) * start.y + 2 * (1 - ease) * ease * control.y + Math.pow(ease, 2) * flight.end.y;
    scale = 2 - (ease * 1); rotate = -20 + (ease * 20);
  }
  const d = pathPoints.length > 1 ? `M ${pathPoints[0].x} ${pathPoints[0].y} ` + pathPoints.map(pt => `L ${pt.x} ${pt.y}`).join(' ') : '';

  return (
    <>
      <svg style={{position:'fixed',inset:0,pointerEvents:'none',zIndex:9998}}>
        <defs>
          <linearGradient id="vibe-grad" x1="0%" y1="0%" x2="100%" y2="0%">
            <stop offset="0%" stopColor="#009bff" stopOpacity="0" />
            <stop offset="60%" stopColor="#009bff" stopOpacity="0.3" />
            <stop offset="100%" stopColor="#770bff" stopOpacity="0.7" />
          </linearGradient>
          <filter id="neon-glow"><feGaussianBlur stdDeviation="4" result="blur" /><feComposite in="SourceGraphic" in2="blur" operator="over" /></filter>
        </defs>
        <path d={d} fill="none" stroke="url(#vibe-grad)" strokeWidth="8" strokeLinecap="round" strokeLinejoin="round" filter="url(#neon-glow)" style={{opacity: pLocal > 0.95 ? 0 : 1, transition: 'opacity 0.2s'}} />
      </svg>
      <div style={{position:'fixed',left:x,top:y,transform:`translate(-50%,-50%) scale(${scale}) rotate(${rotate}deg)`,zIndex:9999,pointerEvents:'none',fontSize:'32px',filter:`drop-shadow(0 0 15px rgba(119, 11, 255, ${pLocal > 0.3 ? 0.5 : 0}))`}}>
        {flight.emoji}
      </div>
    </>
  );
...
const GlobalStyles = () => (
  <style>{`
    ...
    *,*:before,*:after{box-sizing:border-box;margin:0;padding:0}
    ...
    .snap-target-animate { animation: snap-impact 0.6s cubic-bezier(0.34, 1.56, 0.64, 1) forwards; }
    ...
    @keyframes emojiSpring{ 0% {transform:scale(1) rotate(0deg);} 22% {transform:scale(1.45) rotate(-10deg);} 48% {transform:scale(0.84) rotate(7deg);} 68% {transform:scale(1.18) rotate(-4deg);} 84% {transform:scale(0.97) rotate(1deg);} 100%{transform:scale(1) rotate(0deg);} }
    .emoji-pop{ animation:emojiSpring 0.65s cubic-bezier(0.34,1.46,0.64,1) both; display:inline-block; transform-origin:center center; }
    ...
    .n-av-wrap{will-change:transform; transition: transform 0.3s cubic-bezier(0.34, 1.56, 0.64, 1); }
    .n-av-wrap.is-me:hover { transform: scale(1.1); }
...
export default function App() {
  ...
  const getColX = (colIndex, containerWidth, scrollLeft) => {
    const totalTableWidth = Math.max(containerWidth, 860);
  ...
  useEffect(() => {
    const measureContainer = () => { if (scrollRef.current) setTblContainerWidth(scrollRef.current.clientWidth); };
    measureContainer();
    ...
    const decay = () => {
      ...
      if (el) {
        ...
        if (lvl > 0.001) {
          ...
          el.style.boxShadow = [`inset 0 0 ${i1}px ${s1}px rgba(0,155,255,${op1})`,`inset 0 0 ${i2}px ${s2}px rgba(119,11,255,${op2})`,`inset 0 0 ${i3}px ${s3}px rgba(0,229,255,${op3})`,].join(',');
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      supabase.channel('statuses-changes').on('postgres_changes',{event:'*',schema:'public',table:'statuses'},payload=>{
      ...
      supabase.channel('emotions-changes').on('postgres_changes',{event:'*',schema:'public',table:'emotions'},payload=>{
  ...
  useEffect(() => {
    ...
    channel.on('presence',{event:'sync'},()=>{
        ...
        if (status==='SUBSCRIBED') await channel.track({id:meStaffLocal?.id||'guest',name:meStaffLocal?.name||account.name,email:account.username.toLowerCase()});
      });
  ...
  useEffect(() => {
    ...
    return () => { window.removeEventListener('resize', adjustPillPadding); window.removeEventListener('scroll', adjustPillPadding, true); };
  ...
  // Vibe 飞行魔法触发器
  const startVibeFlight = (emo, e) => {
    if (!myAvatarRef.current) return;
    const rect = e.currentTarget.getBoundingClientRect();
    const avRect = myAvatarRef.current.getBoundingClientRect();
    const startPos = { x: rect.left + rect.width / 2, y: rect.top + rect.height / 2 };
    const endPos = { x: avRect.right - 8, y: avRect.bottom - 8 };
    setFlight({ emoji: emo, start: startPos, end: endPos });
    setSocialMenu(null);
  };

  const handleFlightComplete = async () => {
    if (!flight || !meStaff) return;
    const emoji = flight.emoji;
    setEmotions(prev => ({ ...prev, [meStaff.id]: emoji }));
    setSnapEffect(true);
    setFlight(null);
    await supabase.from('emotions').upsert({ staff_id: meStaff.id, emoji: emoji });
  ...
  const inOffice=(()=>{
    let n=0; staffList.forEach(s=>{ if (!(!!records[`${s.id}-${today}-AM`]&&!!records[`${s.id}-${today}-PM`])) n++; });
  ...
  return (
    <div style={{minHeight:'100vh',background:'#f4f5f7'}} onMouseUp={handleStatusCellMouseUp}>
      ...
      <EmojiFlyLayer flight={flight} onComplete={handleFlightComplete} />
      ...
      {activeTab==='calendar' && tblContainerWidth > 0 && nonEditableCols.map(d=>{
        ...
        return (
          <div key={d.ds} style={{position:'fixed',left:x,top:y,transform:'translate(-50%,-50%)',display:'flex',flexDirection:'column',alignItems:'center',gap:'8px',pointerEvents:'none',zIndex:200}}>
            <span key={isBouncing?`${d.ds}-b`:d.ds} className={isBouncing?'emoji-pop':''} style={{fontSize:'48px',userSelect:'none',display:'inline-block',lineHeight:1}}>
              ...
              <div style={{maxHeight:'340px',overflowY:'auto'}}>
                {plannerList().map(h=>(
                  <div key={h.date} className="plan-row" style={{background:h.isWE?'#f0f9ff':'#f9fafb',borderLeft:`3px solid ${h.isWE?'#009bff':'#e5e7eb'}`}} onClick={()=>jumpToDate(h.date)}>
                    <div><div className="plan-date" style={{color:h.isWE?'#009bff':'#374151'}}>{h.date}</div><div style={{fontSize:'10px',color:'#9ca3af',marginTop:'1px'}}>{h.day}</div></div>
          ...
          {onlineUsers.length>0&&(
            <div style={{display:'flex',alignItems:'center',gap:'6px'}}>
              ...
              <div style={{display:'flex',alignItems:'center'}}>
                ...
                {onlineUsers.length>6&&<div style={{marginLeft:-8,width:26,height:26,borderRadius:'50%',background:'#e5e7eb',display:'flex',alignItems:'center',justifyContent:'center',fontSize:'10px',fontWeight:'700',color:'#6b7280',border:'2px solid #fff',zIndex:1}}>+{onlineUsers.length-6}</div>}
              </div>
            </div>
          )}
      ...
      <div className="sub-header"><div className="page-title">Whereabouts</div>{superUser&&<div className="region-toggle">{['Hong Kong','China'].map(r=>(<button key={r} className={`region-btn ${region===r?'on':'off'}`} onClick={()=>setRegion(r)}>{r}</button>))}</div>}</div>

      <div className="toolbar"><button className="tb-btn icon" onClick={()=>navigateWeek(-7)}>‹</button><button className="tb-btn today" onClick={()=>navigateWeek(0,new Date())}>Today</button><button className="tb-btn icon" onClick={()=>navigateWeek(7)}>›</button><select className="tb-select" value={viewDate.getMonth()} onChange={e=>{const d=new Date(viewDate); d.setMonth(+e.target.value); d.setDate(1); navigateWeek(0,d);}}>{Array.from({length:12}).map((_,i)=><option key={i} value={i}>{new Date(0,i).toLocaleString('default',{month:'long'})}</option>)}</select><span className="tb-month">{viewDate.toLocaleString('default',{month:'long',year:'numeric'})}</span></div>

      <div className="legend"><div className="leg-item"><div className="leg-sw" style={{background:'linear-gradient(135deg,#fdf2f8,#fce7f3)',border:'1.5px solid #f9a8d4'}}></div>Holiday</div><div className="leg-item"><div className="leg-sw" style={{background:'linear-gradient(135deg,#eff6ff,#dbeafe)',border:'1.5px solid #93c5fd'}}></div>Weekend</div><div className="leg-item"><div className="leg-sw" style={{background:'linear-gradient(135deg,#e8f0fe,#ede8fe)'}}></div>My days</div><div className="leg-item"><div className="leg-sw" style={{background:'#fafafa',border:'1.5px solid #f3f4f6'}}></div>Team days</div></div>

      <div className="tbl-outer dsz">
        <div className="tbl-hdr-sticky"><div ref={headerRef} className={`tbl-hdr-row ${slideClass}`}><div className="tbl-hdr-namecol"/>{week.map(d=>(<div key={d.ds} className="tbl-hdr-daycol"><div style={{fontSize:'10px',fontWeight:'600',letterSpacing:'0.06em',marginBottom:'5px',color:d.isToday?'#770bff':'#9ca3af'}}>{d.dayName.toUpperCase()}</div><div style={{width:'30px',height:'30px',borderRadius:'50%',margin:'0 auto',display:'flex',alignItems:'center',justifyContent:'center',background:d.isToday?'linear-gradient(135deg,#009bff,#770bff)':'transparent',color:d.isToday?'#fff':'#111827',fontSize:'14px',fontWeight:'600'}}>{d.num}</div></div>))}</div></div>
        ...
        <div ref={scrollRef} className="tbl-scroll dsz" onScroll={handleTableScroll} onMouseLeave={handleStatusCellMouseUp}>
          <table className={`main-tbl ${slideClass}`}>
            <colgroup><col style={{width:'200px'}}/>{week.map(d=><col key={d.ds}/>)}</colgroup>
            <tbody>
              {staffList.map((m,rowIdx)=>{
                ...
                return (
                  <tr key={m.id} id={isMe?'my-row':undefined}>
                    <td className="sticky-c" style={{background:'#f4f5f7',padding:'0 8px 0 0'}}>
                      <div className="nw">
                        <div style={{display:'flex',alignItems:'center',gap:'10px',position:'relative',cursor:isMe?'pointer':'default'}}
                          onClick={async()=>{ if (!isMe) return; if (emotions[m.id]) { await supabase.from('emotions').delete().eq('staff_id',m.id); } else { setSocialMenu(socialMenu===m.id?null:m.id); } }}>
                          <div id={`av-${m.id}`} ref={isMe ? myAvatarRef : null} className={`n-av-wrap ${isMe ? 'is-me' : ''}`} style={{position:'relative'}}>
                            <Avatar name={m.name} photoUrl={staffPhotos[m.id]} size={34} isMe={isMe}/>
                            {emotions[m.id] && !flight && <div className={`emo-tag ${isMe && snapEffect ? 'snap-target-animate' : ''}`}>{emotions[m.id]}</div>}
                          </div>
                          <div><div className={`n-name${isMe?' me':''}`}>{m.name}</div>{isMe&&<div className="n-you">YOU</div>}</div>
                          {isMe&&socialMenu===m.id&&(
                            <div className="emo-picker dsz">
                              {['🧘','⚡','☕','🎯','🚀','💪','🌱'].map(emo=>(
                                <div key={emo}
                                  onClick={e => { e.stopPropagation(); startVibeFlight(emo, e); }}
                    ...
                    {week.map((d,weekIdx)=>{
                      if (!d.editable) { if (!isFirst) return null; const isHol=!!d.hol; return (<td key={d.ds} className="ptd" rowSpan={staffList.length}><div data-pill-ds={d.ds} className={`pill ${isHol?'hol':'we'}`} onClick={e=>fireParty(e,isHol?'holiday':'weekend',d.hol||'',d.ds)}><div className="pill-card"/></div></td>); }
                      return (<td key={d.ds}><div className="dw">{['AM','PM'].map((shift,si)=>{ const key=`${m.id}-${d.ds}-${shift}`; const sid=records[key]||'none'; const cfg=STATUS_CONFIG[sid]; const open=activeMenu===key; const isPreview=isPreviewCell(m.id,weekIdx,shift); const cls=!isMe?'sh other':sid!=='none'?'sh set':'sh mine'; return (<div key={shift} style={{position:'relative'}}><div id={isFirst&&si===0?AM_REF:undefined} className={`${cls} ${isPreview?'preview':''}`} style={sid!=='none'?{background:cfg.bg,color:cfg.color,border:`1.5px solid ${cfg.color}30`}:{}} onMouseDown={e=>handleStatusCellMouseDown(m.id,weekIdx,shift,sid,e)} onMouseOver={()=>handleStatusCellMouseOver(m.id,weekIdx,shift)} onClick={e=>{ if (!isMe) return; e.stopPropagation(); if (preview.length<=1) { if (sid!=='none') handleStatus(key,null,e); else setActiveMenu(open?null:key); } }} > {sid!=='none'?`${cfg.icon} ${cfg.label}`:shift} </div>{open&&isMe&&(<div className="s-drop dsz"><div style={{padding:'3px 10px 7px',fontSize:'10px',color:'#9ca3af',fontWeight:'600',borderBottom:'1px solid #f0f0f0',marginBottom:'3px',letterSpacing:'0.06em'}}>STATUS</div>{Object.entries(STATUS_CONFIG).map(([sId,sCfg])=>(<div key={sId} className="s-opt" onClick={e=>handleStatus(key,sId,e)}><span style={{fontSize:'15px'}}>{sCfg.icon}</span><span className="s-opt-lbl">{sCfg.label}</span></div>))}</div>)}</div>); })}</div></td>);
                    })}
                  </tr>
                );
              })}
