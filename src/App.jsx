  ...
  return (
    <div className="ms-screen"><GlobalStyles />
      <div className="ms-card">
        ...
        <div className="ms-fake-input" onClick={onLogin}>firstname.lastname@patternasia.com</div>
        {isInitializing ? <div style={{fontSize:'13px',color:'#605e5c'}}>Initializing...</div> : <button className="ms-btn" onClick={onLogin}>Next</button>}
...
export default function App() {
    ...
    (async () => {
      ...
      supabase.channel('emotions-changes')
        .on('postgres_changes', { event:'*', schema:'public', table:'emotions' }, payload => {
          setEmotions(e => ({ ...e, [payload.new.staff_id]:payload.new.emoji }));
        }).subscribe();
    })();
  }, [account]);

  // ── Presence + party broadcast ────────────────────────────────────────────
  useEffect(() => {
    if (!account) return;
  ...
  // ── Close dropdowns ───────────────────────────────────────────────────────
  useEffect(() => {
    const fn = e => { if (!e.target.closest('.dsz') && !e.target.closest('.nav-tab')) { setActiveMenu(null); setSocialMenu(null); setActiveTab('calendar'); } };
                    ...
                    <td className="sticky-c" style={{background:'#f4f5f7',padding:'0 8px 0 0'}}>
                      <div className="nw">
                        <div style={{display:'flex',alignItems:'center',gap:'10px',position:'relative',cursor:isMe?'pointer':'default'}}
                          onClick={() => isMe ? setSocialMenu(socialMenu===m.id?null:m.id) : null}>
